    ...
    public int[][] sortRaggedArray(int[][] arr) {
        ...
        for (int i = 0; i < arr.length; i++){
            for (int j = 0; j < arr[i].length; j++){
                //Here we sort values in subarrays
                //We find lowest out of remainder elements and switch it with the current (j)
                int idx = j;
                int lowestElement = arr[i][j];
                for (int k = j + 1; k < arr[i].length; k++){
                    if (arr[i][k] < lowestElement) {
                        lowestElement = arr[i][k];
                        idx = k;
                    }
                }
                int temp = arr[i][j];
                arr[i][j] = lowestElement;
                arr[i][idx] = temp;
            }
        }
        //Separately, we sort subarrays by their length
        for (int i = 0; i < arr.length; i++){
            int idx = i;
            int[] shortestSubArray = arr[i];
            for (int j = i + 1; j < arr.length; j++){
                if (arr[j].length < shortestSubArray.length) {
                    shortestSubArray = arr[j];
                    idx = j;
                }
            }
            int[] temp = arr[i];
            arr[i] = shortestSubArray;
            arr[idx] = temp;
        }
        return arr;
    }
